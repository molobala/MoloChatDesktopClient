...
import com.molo.audio.AudioController;
import com.molo.audio.AudioPlayListener;
...
import com.molo.net.ClienProcessAdapter;
import com.molo.net.Client;
import com.molo.net.ClientProcessListener;
import com.molo.net.MCQuery;
import com.molo.net.MCQueryManager;
import com.molo.net.MediaFile;
import com.molo.net.OnMCQueryResult;
import com.molo.net.Response;
					...
					case MyStandarMessageType.MESSAGE:
						{
							...
							try {
								m = mapper.readValue(data, ChatMessage.class);
					...
					case MyStandarMessageType.ON_MEMBER_DISCONNECTION:
						{
							Membre m;
							try {
								m = mapper.readValue(data, Membre.class);
			...
			public void onSuccess(Response r) {
				...
				if(r.status==MyStandarMessageType.OK){
					...
					if(th!=null){
						...
						try {
							ChatMessage[] messages=mapper.readValue((JsonNode)r.data, ChatMessage[].class);
		...
		q.callback=new OnMCQueryResult() {
			...
			public void onSuccess(Response r) {
				...
				if(r.status==MyStandarMessageType.OK){
					...
					mChatThreadList.clear();
					try {
						SecondChatThread[] threads=mapper.readValue((JsonNode)r.data, SecondChatThread[].class);
				...
				q.callback=new OnMCQueryResult() {
					...
					public void onSuccess(Response r) {
						...
						if(r.status==MyStandarMessageType.AUTHENTIFICATED_SUCCESSFULLY){
							...
							try {
								mMemberObject=mapper.readValue((JsonNode)r.data, Membre.class);
								SwingUtilities.invokeLater(()->{
		...
		if(isConnectedToServer){
			...
			q.callback=new OnMCQueryResult() {
				...
				public void onSuccess(Response r) {
					// TODO Auto-generated method stub
					try {
						Membre[] tmps=mapper.readValue((JsonNode)r.data, Membre[].class);
		...
		q.callback=new OnMCQueryResult() {
			...
			public void onSuccess(Response r) {
				...
				if(r.status!=MyStandarMessageType.SEND_MESSAGE_ERROR){					
					ChatMessage message;
					try {
						message = mapper.readValue((JsonNode)r.data, ChatMessage.class);
						...
						q.callback=new OnMCQueryResult() {
							...
							public void onSuccess(Response rep) {
								...
								if(rep.status==MyStandarMessageType.OK){
									...
									csarKey=new String(RSAEncryption.decrypt(Base64.getDecoder().decode(keyStr),publicKey),StandardCharsets.UTF_8);
